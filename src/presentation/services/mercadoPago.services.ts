    ...
    try {
      ...
      const newPreference = await preference.create({
        body: {
          ...
          notification_url: `https://ea4a-190-228-241-76.ngrok-free.app/api/payment-mercadopago/webhook/?email_acount=${buyerInfo.email}&preference_id=${customPreferenceId}`,
        },
      });
  ...
  static async reciveWebhook(paymentQuery: any) {
    try {
      if (paymentQuery.type === "payment") {
        ...
        if (status === "approved") {
          let buyer = await BuyersHistorynModel.findOne({email:buyerInfo.email});
